...
tags: [chart, visualization, pemeriksaan]
version: "1.0.0"
---
...
Trigger rules — **a chart is part of a data answer, not an extra**:
...
- **Skip the chart only when there is nothing to draw**:
  - the answer is definitional or explanatory — prose, with no data behind it
  - the answer is a single figure with nothing to compare it against
  - the result is a record lookup or an identifier listing
...
**all of them**, not just the total.
...
code. Reshape so each code becomes rows tagged in one series column:
...
- **say once** that the chart could not be displayed, plainly and without
  apology;
- **never call the tool again** to force it;
...
- **One chart per question — but one chart carries many series.** "One chart"
  ...
  showing four codes is one chart.
- **Never re-type tool output into `data=`/`columns=`.** Chart computed results
  by charting the **exported CSV** instead.
